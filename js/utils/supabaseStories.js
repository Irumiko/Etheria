...
(function (global) {
    ...
    async function deleteStory(storyId) {
        ...
        try {
            // Usamos fetch con _writeHeaders() para que el JWT del usuario llegue a RLS.
            // El cliente SDK no siempre propaga la sesión en este contexto.
            // ON DELETE CASCADE maneja la limpieza de mensajes, participantes, etc.
            const headers = await _writeHeaders();
            const url = `${SB_URL}/rest/v1/stories?id=eq.${encodeURIComponent(storyId)}`;
